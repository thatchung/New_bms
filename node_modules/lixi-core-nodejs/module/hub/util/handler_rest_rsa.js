...
let { encryptRSA, signRSA } = require('../../util/_index');
...
module.exports = async function(param, options) {
    ...
    let public_key = await this.public_key_handler({ header: input.headers });
    let private_key = await this.private_key_handler({ header: input.headers });

    let encrypted_data = encryptRSA({
        public_key,
        data: JSON.stringify(param)
    });

    let sign_data;
    if (this.signature_handler)
        sign_data = this.signature_handler({
            encrypted_data,
            header: input.headers
        });
    else sign_data = encrypted_data;

    let signature = signRSA({
        private_key,
        data: sign_data
    });
